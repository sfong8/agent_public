...
Webscraper= Agent(
            ...
            goal=dedent("""Given a list of URLs from the Lead News Investigator, extract the full text content from each article, focusing on information directly related to {company_name}.  Return the extracted text along with the source URL for each article."""),
            tools=[scrape_tool],
            allow_delegation=False,
            verbose=True,
            llm=llm,
